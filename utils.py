...
def step_cost(action):
    if action == UD:  # Unlock Door
        return 1  # Make it inexpensive to encourage unlocking when necessary
    elif action == PK:  # Pickup Key
        return 1 
    elif action in (MF, TL, TR):  # Move and turn actions
        return 2  # Slightly higher cost
    else:
        return 0  # Default
...
def draw_gif_from_seq(seq, env, path="./gif/doorkey.gif"):
    ...
    """
    with imageio.get_writer(path, mode="I", duration=2) as writer:
        img = env.render()
